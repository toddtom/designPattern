...
import interpreter.model.ParserModel;
...
public class Parser {
   ...
   * @author: tao kai (tao.kai@sccaptain.com.cn)
   */
  public static ReadXmlExpression parser(String expr){
    //先初始化记录需解析的元素名称的集合
  ...
  /*=======================第一步=======================*/
  /**
   * 按照从左到右的顺序来分解表达式，得到需要解析的元素名称，还有该元素对应的解析模型
   * @param: expr 需要分解的表达式
  ...
  private static Map<String,ParserModel> parseMapPath(String expr){
    ...
    while (tokenizer.hasMoreTokens()){
      ...
      if(tokenizer.hasMoreTokens()){
        //还有下一个值，说明不是最后一个元素，按照现有的语法，属性必然在最后，因此也不是属性
        setParsePath(false,onePath,false,mapPath);
      }else{
        //说明到最后了
        int dotIndex=onePath.indexOf(DOT);
        if(dotIndex>0){
          //说明是要获取属性的值，那就按照"."来分割，前面的就是元素名字，后面的就是属性名字
  ...
  /**
   * 按照分解出来的位置和名称来设置需要解析的元素名称还有该元素对应的解析模型
   * @param: end 是否是最后一个
   * @param: ele 元素名称
   * @param: propertyValue 是否取属性
   * @param: mapPath 设置需要解析的元素名称还有该元素对应的解析模型
   * @return: returns
   *
   * @since 1.0
   * @version 1.0
  ...
  /*=======================第二步=======================*/
  /**
   * 把分解出来的元素名称根据对应的解析模型转换成为对应的解释器对象
  ...
  private static ReadXmlExpression buildTree(List<ReadXmlExpression> list){
    ...
    for(ReadXmlExpression re:list){
      if(preRe==null){
        ...
        //把元素添加到上一个对象下面，同时把本对象设置为oldRe作为下一个对象的父节点
